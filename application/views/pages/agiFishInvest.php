<!DOCTYPE html>
<html class="no-js">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <title>Agivest | Smart Social Investment</title>
        <!-- for Google -->
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="description" content="Investasi cerdas berjiwa kerakyatan dengan sistem gotong royong."/>
        <meta name="keywords" content="agivest, agivest.com, agi-fish, agi-livestock, investasi, platform investasi, agivest Indonesia" />
        <meta name="author" content="Agivest Team" />
        <meta name="copyright" content="Agivest" />
        <meta name="application-name" content="agivest.com" />

        <!-- for Facebook -->
        <meta property="og:url" content="https://www.agivest.com/" />
        <meta property="og:type" content="article" />
        <meta property="og:title" content="agivest.com" />
        <meta property="og:description" content="Agivest smart social investment, Investasi cerdas berjiwa kerakyatan dengan sistem gotong royong." />
        <meta property="og:image" content="https://www.agivest.com/asset/images/agivestLogo.png" />

        <!-- for Twitter -->
        <meta name="twitter:card" content="summary" />
        <meta name="twitter:title" content="Agivest" />
        <meta name="twitter:description" content="Agivest smart social investment, Investasi cerdas berjiwa kerakyatan dengan sistem gotong royong." />
        <meta name="twitter:image" content="https://www.agivest.com/asset/images/agivestLogo.png" />

        <link rel="apple-touch-icon" sizes="57x57" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-57x57.png"> 
        <link rel="apple-touch-icon" sizes="60x60" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-60x60.png"> 
        <link rel="apple-touch-icon" sizes="72x72" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-72x72.png"> 
        <link rel="apple-touch-icon" sizes="76x76" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-76x76.png"> 
        <link rel="apple-touch-icon" sizes="114x114" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-114x114.png"> 
        <link rel="apple-touch-icon" sizes="120x120" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-120x120.png"> 
        <link rel="apple-touch-icon" sizes="144x144" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-144x144.png"> 
        <link rel="apple-touch-icon" sizes="152x152" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-152x152.png"> 
        <link rel="apple-touch-icon" sizes="180x180" href="<?php echo base_url(); ?>assets/images/favicon/apple-touch-icon-180x180.png"> 
        <link rel="icon" type="image/png" href="<?php echo base_url(); ?>assets/images/favicon/favicon-32x32.png" sizes="32x32"> 
        <link rel="icon" type="image/png" href="<?php echo base_url(); ?>assets/images/favicon/favicon-194x194.png" sizes="194x194"> 
        <link rel="icon" type="image/png" href="<?php echo base_url(); ?>assets/images/favicon/favicon-96x96.png" sizes="96x96"> 
        <link rel="icon" type="image/png" href="<?php echo base_url(); ?>assets/images/favicon/android-chrome-192x192.png" sizes="192x192"> 
        <link rel="icon" type="image/png" href="<?php echo base_url(); ?>assets/images/favicon/favicon-16x16.png" sizes="16x16"> 


        <link href='https://fonts.googleapis.com/css?family=PT+Sans:400,700,400italic,700italic' rel='stylesheet' type='text/css'>

        <!-- Animate.css -->
        <link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/animate.css">
        <!-- Font Awesome -->
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.5.0/css/font-awesome.min.css">
        <!-- Bootstrap  -->
        <link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/bootstrap.css">
        <!-- Owl Carousel  -->
        <link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/owl.carousel.min.css">
        <link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/owl.theme.default.min.css">
        <!-- Style -->
        <link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/style.css">

        <!-- Modernizr JS -->
        <script src="<?php echo base_url(); ?>assets/js/modernizr-2.6.2.min.js"></script>
        <!-- Google Analytics-->
        <script>
          (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
          (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
          m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
          })(window,document,'script','https://www.google-analytics.com/analytics.js','ga');
          ga('create', 'UA-82776023-1', 'auto');
          ga('send', 'pageview');
        </script>

        <!-- Chatra {literal} -->
        <script>
            (function(d, w, c) {
                w.ChatraID = 'yJRrSfAyeYptA8Em7';
                var s = d.createElement('script');
                w[c] = w[c] || function() {
                    (w[c].q = w[c].q || []).push(arguments);
                };
                s.async = true;
                s.src = (d.location.protocol === 'https:' ? 'https:' : 'http:')
                        + '//call.chatra.io/chatra.js';
                if (d.head)
                    d.head.appendChild(s);
            })(document, window, 'Chatra');
        </script>
    </head>
    <body>
        <header role="banner" id="agivest-header">
            <div class="fluid-container">
                <nav class="navbar navbar-default">
                    <div class="navbar-header">
                        <!-- Mobile Toggle Menu Button -->
                        <a href="#" class="js-agivest-nav-toggle agivest-nav-toggle" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar"><i></i></a>
                        <a class="navbar-brand" href="<?php echo base_url(); ?>">
                            <img src="<?php echo base_url(); ?>assets/images/agivestLogo.png" style="height:32px; margin-top: -5px;" alt="Agivest logo">
                        </a> 
                    </div>
                    <div id="navbar" class="navbar-collapse collapse">
                        <ul class="nav navbar-nav navbar-right">
                            <li><a href="<?php echo base_url();?>"><span>Home</span></a></li>
                            <li><a href="<?php echo site_url("TentangKami");?>" ><span>Tentang Kami</span></a></li>
                            <?php 
                                if (empty($namaAdmin)) {?>
                                <li class="call-to-action">
                                    <a class="log-in" href="<?php echo site_url('Login');?>"><span>Masuk</span></a>
                                </li>
                                <li class="call-to-action"><a class="sign-up" href="<?php echo site_url('Register');?>"><span>Daftar</span></a></li>
                            <?php    
                                }else{?>
                                <li class="dropdown">
                                    <a href="#" id="profile" data-toggle="dropdown"><?php echo $namaAdmin;?><span class="caret"></span></a>
                                    <ul class="dropdown-menu" role="menu" aria-labelledby="profile">
                                        <li role="presentation">
                                            <a href="<?php echo site_url("Investor");?>">Dashboard</a>
                                        </li>
                                        <li role="presentation"><a href="<?php echo site_url('Login/logout'); ?>">Keluar</a></li>
                                    </ul>
                                </li>
                            <?php
                                }
                            ?>
                        </ul>
                    </div>
                </nav>
            </div>
        </header>

        <!-- KONTEN -->
        <section id="agivest-services">
            <div class="agivest-services">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12 section-heading text-center">
                            <h2 class="to-animate"><span>Konfirmasi Investasi</span></h2>
                            <div class="row">
                                <div class="col-md-8 col-md-offset-2 subtext">
                                    <img class="to-animate img-responsive"src="<?php echo base_url();?>assets/images/agiStep_1.png">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-5">
                            <div class="row">
                                <div class="col-md-12">
                                    <img class="to-animate img-thumbnail img-responsive" src="<?php echo base_url();?>assets/images/perikanan.jpeg">
                                    <div class="core">
                                        <div class=" to-animate project-collected">
                                            <b> 
                                                <strong>Rp. 0,-</strong>
                                             
                                                <span> Terkumpul</span></b>
                                        </div>
                                        <div class="to-animate project-bar">
                                            <span class="project-bar-progress" style="width: 2%">
                                            </span>
                                        </div>
                                        <div class="to-animate project-target">
                                           <b><strong>0%</strong></b>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-7">
                            <div class="row">
                                <div class="col-md-4">
                                    <h3 class="to-animate" style="text-align: center;">Deskripsi</h3>
                                    <hr class="to-animate" style="margin-top: -20px;">
                                    <p class="to-animate"><b><i class="fa fa-anchor"></i>   Nama Kolam :</b> Agi-1</p>
                                    <p class="to-animate"><b><i class="fa fa-map-marker"></i>   Lokasi :</b> <a href="#">Malang, Jawa Timur</a></p>
                                    <p class="to-animate"><b><i class="glyphicon glyphicon-signal"></i>   Luas :</b> 3m x 3m x 1m</p>
                                    <p class="to-animate"><b><i class="fa fa-money"></i>   Total Investasi :</b> Rp. 20.000.000,-</p>
                                </div>
                                <div class="col-md-8">
                                    <h3 class="to-animate" style="text-align: center;">Perhitungan Keuntungan</h3>
                                    <hr class="to-animate" style="margin-top: -20px;">
                                    <form class="to-animate">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <select name="investasi" id="investasi" class="form-control">
                                                    <option value="0">Rp. 200.000,-</option>
                                                    <option value="1">Rp. 400.000,-</option>
                                                    <option value="2">Rp. 600.000,-</option>
                                                    <option value="3">Rp. 800.000,-</option>
                                                    <option value="4">Rp. 1.000.000,-</option>
                                                    <option value="5">Rp. 2.000.000,-</option>
                                                    <option value="6">Rp. 3.000.000,-</option>
                                                    <option value="7">Rp. 4.000.000,-</option>
                                                    <option value="8">Rp. 5.000.000,-</option>
                                                    <option value="9">Rp. 6.000.000,-</option>
                                                    <option value="10">Rp. 7.000.000,-</option>
                                                    <option value="11">Rp. 8.800.000,-</option>
                                                    <option value="12">Rp. 9.000.000,-</option>
                                                    <option value="13">Rp. 10.000.000,-</option>
                                                    <option value="14">Rp. 11.000.000,-</option>
                                                    <option value="15">Rp. 12.000.000,-</option>
                                                    <option value="16">Rp. 13.000.000,-</option>
                                                    <option value="17">Rp. 14.000.000,-</option>
                                                    <option value="18">Rp. 15.000.000,-</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <b>
                                                        <textarea disabled placeholder="Return Invest" id="return-Invest" name="return-Invest" class="form-control" rows="1" ></textarea>
                                                    </b>
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <h4>Lama Investasi <strong>3.5 Tahun</strong></h4>
                                            </div>
                                            <div class="col-md-12" style="margin-top: -20px;">
                                                <div class="form-group">
                                                    <button type="button" onclick="myFunction()" id="btn-simulasi" class="btn btn-info" style="margin-top: 2%"><span class="fa fa-search"></span> Simulasi</button>                                                
                                                </div>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                            </div>
                            <h3 class="to-animate" style="text-align: center;">Investasi Sekarang</h3>
                            <hr class="to-animate" style="margin-top: -20px;">
                            <!--<form action="<?php echo site_url('AgiFishInvest/agiFishInvestStep2');?>" method="post" class="to-animate">
                                -->
                                <form method="post" class="to-animate"> 
                                <div class="row">
                                    <div class="col-md-12" style="margin-top: -20px;">
                                        <label style="font-size: 17px;">Pilih nominal invest: </label>
                                        <div class="form-group">
                                            <input type="hidden" name="namaKolam" id="namaKolam" value="
                                            <?php 
                                            if (!empty($namaKolam)) {
                                                echo $namaKolam;
                                            }else{
                                                echo null;
                                            }?>">
                                            <select name="modalInvestasi" id="modalInvestasi" class="form-control">
                                            <option value="200000">Rp. 200.000,-</option>
                                            <option value="400000">Rp. 400.000,-</option>
                                            <option value="600000">Rp. 600.000,-</option>
                                            <option value="800000">Rp. 800.000,-</option>
                                            <option value="1000000">Rp. 1.000.000,-</option>
                                            <option value="2000000">Rp. 2.000.000,-</option>
                                            <option value="3000000">Rp. 3.000.000,-</option>
                                            <option value="4000000">Rp. 4.000.000,-</option>
                                            <option value="5000000">Rp. 5.000.000,-</option>
                                            <option value="6000000">Rp. 6.000.000,-</option>
                                            <option value="7000000">Rp. 7.000.000,-</option>
                                            <option value="8000000">Rp. 8.800.000,-</option>
                                            <option value="9000000">Rp. 9.000.000,-</option>
                                            <option value="10000000">Rp. 10.000.000,-</option>
                                            <option value="11000000">Rp. 11.000.000,-</option>
                                            <option value="12000000">Rp. 12.000.000,-</option>
                                            <option value="13000000">Rp. 13.000.000,-</option>
                                            <option value="14000000">Rp. 14.000.000,-</option>
                                            <option value="15000000">Rp. 15.000.000,-</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <!--
                                            <button type="submit" name="submit" id="submit" class="btn btn-info" style="margin-top: 2%"><span class="fa fa-rocket"></span> Lanjutkan</button>
                                            -->
                                         </div>
                                    </div>
                                </div>
                            </form>
                            <button data-toggle="modal" data-target="#myModal" name="submit" id="submit" class="btn btn-info to-animate" style="margin-top: 2%"><span class="fa fa-rocket"></span> Lanjutkan</button>
                                       
                            <!-- Modal -->
                            <div class="modal  fade" id="myModal" role="dialog">
                                <div class="modal-dialog">
                                    <!-- Modal content-->
                                    <div class="modal-content">
                                    <div class="modal-header">
                                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                                        <h4 class="modal-title">Perhatian Sahabat Agivest</h4>
                                    </div>
                                        <div class="modal-body">
                                            <p>Maaf, website sedang proses developing. Kami akan mengkabari melalui email anda jika sudah selesai developing</p>
                                        </div>
                                        <div class="modal-footer" >
                                            <a href="<?php echo site_url("Register");?>" type="button" class="btn btn-default" >NEXT</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <script type="text/javascript">

            function myFunction() {
                var investasi, text;
                var harga = [200000, 400000, 600000, 800000, 1000000, 2000000, 3000000,
                4000000,5000000,6000000,7000000,8000000,9000000,10000000,11000000,12000000,
                13000000,14000000,15000000];
                // Get the value of the input field with id="numb"
                investasi = document.getElementById("investasi").value;
                
                
                for (var i = 0; i <= 18; i++) {
                    if (investasi == i + '') {
                        var totalInvest = 20000000;
                        var modal = harga[i];
                        var kepemilikan = modal / totalInvest;
                        var hasilPanen = 25000000;
                        var hitung = hasilPanen - totalInvest;
                        var hitung2 = hitung * 0.6;
                        var hitung3 = hitung2 * kepemilikan;
                        text = "Rp. " + hitung3 + " / Panen";
                    }
                };
                document.getElementById("return-Invest").innerHTML = text;
            }
   
        </script>

        <div id="agivest-footer" role="contentinfo">
            <div class="container">
                <div class="row">
                    <div class="col-md-3 to-animate">
                        <h3 class="section-title">Tentang Kami</h3>
                        <ul class="agi-footer">
                            <li><a href="<?php echo site_url("TentangKami");?>">Siapa kami</a></li>
                        </ul>
                    </div>
                    <div class="col-md-3 to-animate">
                        <h3 class="section-title">Bantuan</h3>
                        <ul class="agi-footer">
                            <li><a href="<?php echo site_url("Faq");?>">FAQ</a></li>
                            <li><a href="#">Term Of Investment</a></li>
                            <li><a href="#">Term Of Use</a></li>
                            <li><a href="#">Privacy Policy</a></li>
                        </ul>
                    </div>
                    <div class="col-md-3 to-animate">
                        <h3 class="section-title">Investasi Kami</h3>
                        <ul class="agi-footer">
                            <li><a href="#">Agi - Fish</a></li>
                            <li><a href="#">Agi - Pet</a></li>
                        </ul>
                    </div>
                    <div class="col-md-3 to-animate">
                        <h3 class="section-title">Hubungi Kami</h3>
                        <ul class="contact-info">
                            <li>
                                <i class="fa fa-map-marker"></i>Gedung Equaloop
                                Griyashanta Blok E-209
                                Kec. Lowokwaru, Kota Malang, Jawa Timur 65145
                            </li>
                            <li><i class="fa fa-phone"></i>+62 838 7376 7405 (Helfi)</li>
                            <li><i class="fa fa-envelope"></i><a href="#">care@agivest.com</a></li>
                            <li><i class="fa fa-globe"></i><a href="#">www.agivest.com</a></li>
                        </ul>
                        <ul class="social-media">
                            <li><a href="#" class="facebook"><i class="fa fa-facebook"></i></a></li>
                            <li><a href="#" class="twitter"><i class="fa fa-twitter"></i></a></li>
                            <li><a href="#" class="instagram"><i class="fa fa-instagram"></i></a></li>
                            <li><a href="#" class="mail"><i class="fa fa-mail2"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>


        <!-- jQuery -->
        <script src="<?php echo base_url(); ?>assets/js/jquery.min.js"></script>
        <!-- jQuery Easing -->
        <script src="<?php echo base_url(); ?>assets/js/jquery.easing.1.3.js"></script>
        <!-- Bootstrap -->
        <script src="<?php echo base_url(); ?>assets/js/bootstrap.min.js"></script>
        <!-- Waypoints -->
        <script src="<?php echo base_url(); ?>assets/js/jquery.waypoints.min.js"></script>
        <!-- Stellar Parallax -->
        <script src="<?php echo base_url(); ?>assets/js/jquery.stellar.min.js"></script>
        <!-- Owl Carousel -->
        <script src="<?php echo base_url(); ?>assets/js/owl.carousel.min.js"></script>
        <!-- Counters -->
        <script src="<?php echo base_url(); ?>assets/js/jquery.countTo.js"></script>
        <!-- Main JS (Do not remove) -->
        <script src="<?php echo base_url(); ?>assets/js/main.js"></script>
    </body>
</html>