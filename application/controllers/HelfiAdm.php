...
class HelfiAdm extends CI_Controller {
    ...
    public function index() {
       if($this->session->userdata('adminSession')){
            ...
            //Count User
            $data['countUser'] = $this->Admin_model->countTable("fyidentitas", "idIdentitas");

            //Count Tambak
            $data['countTambak'] = $this->Admin_model->countTable("fytambak", "idTambak");

            //Invoice
            $data['Invoice'] = $this->Admin_model->getDataInvoice();

            $this->load->view('pages/Investor/helfiDashboard', $data);    
        }else{
            //Tambak
    ...
    public function agiFish() {
            ...
            //Tambak
            $data['agiTambak'] = $this->Admin_model->getDataTable('fytambak');

            $this->load->view('pages/Investor/agi-fish2', $data);    
        }else{
            //Tambak
            $tambak = $this->Home_model->getData("fytambak", "where statusTambak = 'N' ");
            $namaTambak = "";
    ...
    public function agiPet() {
        if($this->session->userdata('adminSession')){
            ...
            $this->load->view('pages/Investor/agi-pet2', $data);    
        }else{
            //Tambak
            $tambak = $this->Home_model->getData("fytambak", "where statusTambak = 'N' ");
            $namaTambak = "";
    ...
    public function user() {
            ...
            //User
            $agiUser = $this->Admin_model->getDataTable("fyidentitas");
            $data['agiUser'] = $agiUser;
            $this->load->view('pages/Investor/agi-users', $data);    
        }else{
            //Tambak
            $tambak = $this->Home_model->getData("fytambak", "where statusTambak = 'N' ");
            $namaTambak = "";
